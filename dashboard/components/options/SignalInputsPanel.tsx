...
export function SignalInputsPanel({ result }: Props) {
  ...
  const rsiMood: "fires-for" | "fires-against" | "neutral" =
    rsiDaily >= 70
      ? bearishPos ? "fires-for" : "fires-against"
      : rsiDaily <= 30
      ? bullishPos ? "fires-for" : "fires-against"
      : "neutral";
  const rsiLabel =
    rsiDaily >= 70 ? "overbought"
    : rsiDaily <= 30 ? "oversold"
    : rsiDaily >= 60 ? "strong"
    : rsiDaily <= 40 ? "weak"
    : "neutral";

  const macdHist = s.chart_tf.macd_hist;
  const macdMood = macdHist == null
    ? "neutral"
    : macdHist > 0
      ? (bullishPos ? "fires-for" : "fires-against")
      : (bearishPos ? "fires-for" : "fires-against");
  const macdLabel = macdHist == null ? "—" : macdHist > 0 ? "positive" : "negative";

  const smi = s.chart_tf.smi;
  const smiSig = s.chart_tf.smi_signal;
  const smiMood = smi == null
    ? "neutral"
    : smi >= 40
      ? (bearishPos ? "fires-for" : "warn")
      : smi <= -40
      ? (bullishPos ? "fires-for" : "warn")
      : "neutral";
  const smiLabel = smi == null
    ? "—"
    : smi >= 40 ? "overbought"
    : smi <= -40 ? "oversold"
    : smiSig != null && smi > smiSig ? "above signal"
    : smiSig != null && smi < smiSig ? "below signal"
    : "neutral";

  const vwap = s.chart_tf.vwap;
  const isIntradayTF = !["1d", "1w", "1mo"].includes(tf);
  const vwapDiffPct = vwap && vwap > 0 ? ((spot - vwap) / vwap) * 100 : null;
  const vwapMood = !isIntradayTF || vwapDiffPct == null
    ? "neutral"
  ...
  const ivMood = ivRv == null
    ? "neutral"
    : ivRv >= 1.3 && isLong ? "fires-against"
    : ivRv >= 1.3 && !isLong ? "fires-for"
    : ivRv <= 0.8 && isLong ? "fires-for"
  ...
  const dteMood = dte <= 7
    ? (isLong ? "fires-against" : "warn")
    : dte <= 21 && isLong ? "warn"
    : "neutral";
  const deltaMood = absD == null
    ? "neutral"
    : absD >= 0.65 && isLong ? "fires-for"
    : absD < 0.20 && isLong ? "fires-against"
    : absD >= 0.40 && !isLong ? "fires-against"
    : "neutral";
  ...
  const rows: {
  ...
  }[] = [
    {
      ...
      source: "5d model",
      value: er == null ? "—" : `${er >= 0 ? "+" : ""}${er.toFixed(2)}% (±${band?.toFixed(1)}%)`,
      label: er == null ? "unavailable"
        : er >= 3 ? "strong bullish"
        : er >= 0.5 ? "mild bullish"
        : er <= -3 ? "strong bearish"
        : er <= -0.5 ? "mild bearish"
    ...
    { name: "RSI", source: "daily", value: rsiDaily.toFixed(1), label: rsiLabel, mood: rsiMood },
    {
      name: "MACD hist",
      source: tf,
      value: macdHist == null ? "—" : `${macdHist >= 0 ? "+" : ""}${macdHist.toFixed(3)}`,
      label: macdLabel,
      mood: macdMood,
    },
    {
      name: "SMI",
      source: tf,
      value: smi == null ? "—" : `${smi.toFixed(1)}${smiSig != null ? ` / ${smiSig.toFixed(1)}` : ""}`,
      label: smiLabel,
      mood: smiMood,
    },
    {
      name: "VWAP",
      source: tf,
      value: !isIntradayTF ? "n/a daily" :
        vwap && vwap > 0 ? `${vwap.toFixed(2)} (${vwapDiffPct! >= 0 ? "+" : ""}${vwapDiffPct!.toFixed(2)}%)` : "—",
      label: !isIntradayTF ? "intraday only" :
        vwapDiffPct == null ? "—" :
        Math.abs(vwapDiffPct) < 0.3 ? "near vwap" :
        vwapDiffPct > 0 ? "above vwap" : "below vwap",
      mood: vwapMood,
    },
    {
      name: "EMA stack",
    ...
    },
    {
      ...
      source: "expiry",
      value: `${dte}d`,
      label: dte <= 7 ? "expiring" : dte <= 21 ? "mgmt zone" : dte >= 45 ? "early" : "mid",
      mood: dteMood,
    },
    {
      name: "|Δ|",
      ...
      label: absD == null ? "—" :
        absD >= 0.65 ? "deep ITM" :
        absD >= 0.40 ? "near money" :
        absD >= 0.25 ? "OTM" : "far OTM",
      mood: deltaMood,
    },
  ];
          ...
          <tbody>
            {rows.map((r, i) => (
              ...
              >
                ...
                <td className="px-3 py-1.5 text-text-muted text-[10px]">{r.source}</td>
                <td className="px-3 py-1.5 text-right text-text-primary">{r.value}</td>
                <td
                  className={cn(
