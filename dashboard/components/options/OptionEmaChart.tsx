...
export function OptionEmaChart({ prices, ema9, ema21, title, height = 200 }: Props) {
  const W = 700;
  const H = height;
  const PAD_L = 50;
  const PAD_R = 8;
  ...
  const { pricePath, ema9Path, ema21Path, areaPath, ticks, yOf, xOf, hi, lo } = useMemo(() => {
    if (prices.length === 0) {
      return {
        pricePath: "",
        ema9Path: "",
    ...
    const yOf = (v: number) => PAD_T + innerH - ((v - yMin) / (yMax - yMin || 1)) * innerH;
    const xOf = (i: number) => PAD_L + (i / Math.max(prices.length - 1, 1)) * innerW;

    const mkPath = (arr: number[]) =>
      arr.map((v, i) => `${i === 0 ? "M" : "L"}${xOf(i).toFixed(2)},${yOf(v).toFixed(2)}`).join(" ");

    const area =
      `M${xOf(0).toFixed(2)},${(PAD_T + innerH).toFixed(2)} ` +
      prices.map((v, i) => `L${xOf(i).toFixed(2)},${yOf(v).toFixed(2)}`).join(" ") +
    ...
    <div className="relative w-full">
      <div className="flex items-center gap-2 mb-1">
        ...
        <span className="text-[10px] tabular text-text-secondary ml-auto">
          {hover ? (
            <>
              <span className="text-text-muted">{hover.ago === 0 ? "now" : `−${hover.ago}d`}</span>
              <span className="ml-2 text-sky-400">px</span>{" "}
              <span className="text-text-primary font-medium">${hover.price?.toFixed(2)}</span>
              ...
              <span className="text-text-primary">${hover.e21?.toFixed(2)}</span>
            </>
          ) : (
            <>
              last <span className="text-text-primary font-medium">${last?.toFixed(2)}</span>
      ...
      >
        ...
        <path d={pricePath} stroke="rgb(56,189,248)" strokeWidth="1.6" fill="none" />
        <path
          d={ema9Path}
        ...
        {prices.length > 0 && (
          <circle cx={xOf(prices.length - 1)} cy={yOf(last)} r={3} fill="rgb(56,189,248)" />
        )}
        ...
        {hover && (
          <g pointerEvents="none">
            <line
              ...
              strokeDasharray="3 3"
            />
            <circle cx={xOf(hover.i)} cy={yOf(hover.price)} r={3} fill="rgb(56,189,248)" />
            <circle cx={xOf(hover.i)} cy={yOf(hover.e9)}    r={2.5} fill="rgb(34,197,94)" />
            <circle cx={xOf(hover.i)} cy={yOf(hover.e21)}   r={2.5} fill="rgb(251,191,36)" />
          </g>
        )}
      </svg>
