...
function SummaryStrip({ snap }: { snap: MonitorSnapshot | null }) {
  ...
  const stats: [string, string, string?][] = [
    ...
    [
      "warning",
      snap.warning.toString(),
      snap.warning > 0 ? "text-warning" : undefined,
    ],
    [
      "trigger",
      snap.triggered.toString(),
      snap.triggered > 0 ? "text-down" : undefined,
    ],
  ];
  return (
      ...
      <div className="flex flex-col md:flex-row md:items-center md:gap-4 gap-2">
        ...
        <div className="flex items-baseline justify-between md:justify-end gap-4 md:gap-3 text-xs tabular shrink-0">
          <span className="text-text-secondary">
            {entry.quantity}× contract{entry.quantity !== 1 ? "s" : ""}
          </span>
          <span className="text-text-muted">·</span>
          <span className="text-text-muted">
            headroom{" "}
            <span className={cn(meta.tone, "font-medium")}>
  ...
  return (
    <div className="py-16 text-center">
      ...
      <p className="text-xs text-text-muted mt-2">
        When you open a credit spread, it'll appear here and the monitor will
        track its delta every 5 minutes during market hours.
      </p>
    </div>
  );
}
...
function Footer({
  ...
  return (
    <div className="border-t border-border/40 pt-4 flex items-baseline justify-between text-[10px] uppercase tracking-wider text-text-muted">
      <span>
        last checked {lastRun ? new Date(lastRun).toLocaleTimeString() : "—"}
      </span>
