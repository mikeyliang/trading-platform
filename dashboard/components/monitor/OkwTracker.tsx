      ...
      {trades.length === 0 ? (
        <div className="py-12 text-center text-sm text-text-secondary">
          ...
          <div className="text-xs text-text-muted mt-1.5">
            Click <span className="text-text-primary">Add trade</span> after you
            place one to keep a running OKW.
          </div>
        </div>
      ) : (
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-6 gap-x-8 gap-y-4 py-2">
      ...
      <Stat
        label="Open"
        value={String(s.open)}
        tone={s.open > 0 ? "accent" : undefined}
      />
      <Stat label="Closed" value={String(s.closed)} />
      <Stat
        label="Win rate"
        value={s.closed > 0 ? `${winRate}%` : "—"}
        tone={
          winRate >= 70
            ? "up"
            : winRate >= 50
      ...
      <Stat
        label="Wins"
        value={String(s.wins)}
        tone={s.wins > 0 ? "up" : undefined}
      />
      <Stat
        label="Realized $"
        value={fmtCurrency(s.realized_pnl)}
        tone={
          s.realized_pnl > 0 ? "up" : s.realized_pnl < 0 ? "down" : undefined
        }
      />
    </div>
  );
...
function AddForm({
  ...
  const submit = async () => {
    if (
      ...
      setError(
        "Symbol, type, expiry, DTE, both strikes, and credit are required.",
      );
      return;
    }
    setSaving(true);
    ...
    <section className="border border-border/40 rounded-md p-4 bg-surface-2/30 flex flex-col gap-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <FormField label="Type">
          <Select
            value={f.trade_type ?? "mars"}
            onValueChange={(v) => setField("trade_type", v)}
          >
        ...
        <FormField label="Symbol">
          <Input
            ...
            className="h-8 uppercase tabular"
          />
        </FormField>
        ...
        <FormField label="DTE">
          <Input
            ...
            value={f.dte ?? ""}
            onChange={(e) =>
              setField("dte", e.target.value ? +e.target.value : undefined)
            }
            className="h-8 tabular"
          />
        </FormField>
        ...
        <FormField label="Contracts">
          <Input
            ...
            min={1}
            value={f.contracts ?? 1}
            onChange={(e) => setField("contracts", +e.target.value || 1)}
            className="h-8 tabular"
          />
        </FormField>
        <FormField label="Credit $">
          <Input
            ...
            value={f.credit ?? ""}
            onChange={(e) =>
              setField("credit", e.target.value ? +e.target.value : undefined)
            }
            className="h-8 tabular"
          />
        </FormField>
        ...
        <FormField label="AROC %">
          <Input
            ...
            value={f.aroc_pct ?? ""}
            onChange={(e) =>
              setField("aroc_pct", e.target.value ? +e.target.value : undefined)
            }
            className="h-8 tabular"
          />
        </FormField>
  ...
  return (
    <div className="overflow-x-auto -mx-1">
      <table className="w-full text-[11px] tabular">
        ...
        <tbody>
          {trades.map((t) => (
            <Row key={t.id} t={t} onRefresh={onRefresh} />
          ))}
        </tbody>
      </table>
    </div>
  ...
  return (
    <>
      <tr className="border-t border-border/30 hover:bg-surface-2/30">
        ...
        <td
          className={cn(
            "px-3 py-2.5 uppercase text-[10px] tracking-wider",
            tone,
          )}
        >
        ...
        <td className="px-3 py-2.5 text-right text-text-secondary">
          {t.adj_distance_pct != null
            ? `${t.adj_distance_pct.toFixed(1)}%`
            : "—"}
        </td>
        <td className="px-3 py-2.5 text-right text-text-secondary">
          {t.contracts}
        ...
        <td className="px-3 py-2.5">
          ...
          >
            {t.status}
            {t.exit_reason ? (
              <span className="text-text-muted ml-1">· {t.exit_reason}</span>
            ) : null}
          </span>
        </td>
      ...
      {closing && (
        <tr>
          <td
            colSpan={14}
            className="px-3 py-3 bg-surface-2/30 border-t border-border/30"
          >
