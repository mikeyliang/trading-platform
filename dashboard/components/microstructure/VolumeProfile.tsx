...
export function VolumeProfile({ symbol, defaultTimeframe = "15m", defaultDays = 20 }: Props) {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setData(null);
    api.volumeProfile(symbol, tf, days, 40)
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-2 h-7 border-b border-border/60 text-[10px] uppercase tracking-wider text-text-muted">
        ...
        <Select value={tf} onValueChange={setTf}>
          ...
          <SelectContent>
            {["5m", "15m", "30m", "1h", "1d"].map((t) => (
              <SelectItem key={t} value={t} className="text-[10px] tabular">{t}</SelectItem>
            ))}
          </SelectContent>
        </Select>
        <Select value={String(days)} onValueChange={(v: string) => setDays(Number(v))}>
          ...
          <SelectContent>
            {[5, 10, 20, 60].map((d) => (
              <SelectItem key={d} value={String(d)} className="text-[10px] tabular">{d}d</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
      ...
      {loading && !data ? (
        ...
        <div className="flex-1 overflow-auto">
          ...
          {data.bins.slice().reverse().map((b, i) => {
            const isPoc = data.poc != null && b.price_low <= data.poc && data.poc <= b.price_high;
            const inVA = data.value_area_low != null && data.value_area_high != null &&
              b.price_mid >= data.value_area_low && b.price_mid <= data.value_area_high;
            ...
            return (
              ...
              >
                <span className="text-text-secondary">{b.price_mid.toFixed(2)}</span>
                <div className="relative h-full flex items-center">
                  <span
                    className={cn(
                      "h-[10px] rounded-sm",
                      isPoc ? "bg-accent" : inVA ? "bg-accent/60" : "bg-text-muted/40"
                    )}
                    style={{ width: `${Math.max(2, w)}%` }}
                  />
                </div>
                <span className="text-right text-text-muted">{fmtCompact(b.volume)}</span>
              </div>
            );
          })}
        </div>
      ...
      {data && data.poc != null && (
        <div className="flex items-center justify-between px-2 h-6 border-t border-border/60 text-[9px] tabular text-text-muted">
          <span>POC <span className="text-accent font-medium">{data.poc.toFixed(2)}</span></span>
          {data.value_area_low != null && data.value_area_high != null && (
            <span>VA <span className="text-text-secondary font-medium">
              {data.value_area_low.toFixed(2)}–{data.value_area_high.toFixed(2)}
            </span></span>
          )}
