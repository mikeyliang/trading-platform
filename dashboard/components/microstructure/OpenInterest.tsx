...
export function OpenInterestByStrike({ symbol }: Props) {
  ...
  useEffect(() => {
    ...
    setExpiry("");
    setExpirations([]);
    api.optionsChain(symbol).then((meta) => {
      if (cancelled) return;
  ...
  const { rows, callOI, putOI, maxOI, pcr, maxPain, spot } = useMemo(() => {
    if (!chain) {
      return { rows: [], callOI: 0, putOI: 0, maxOI: 1, pcr: null as number | null, maxPain: null as number | null, spot: null as number | null };
    }
    ...
    return { rows, callOI, putOI, maxOI, pcr, maxPain, spot: chain.underlying_price };
  }, [chain]);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-2 px-2 h-7 border-b border-border/60 text-[10px] uppercase tracking-wider text-text-muted">
        ...
        <Select value={expiry} onValueChange={setExpiry} disabled={expirations.length === 0}>
          <SelectTrigger className="h-5 w-24 text-[10px] tabular ml-auto">
            <SelectValue placeholder="—" />
          </SelectTrigger>
          <SelectContent>
            {expirations.map((e) => (
              <SelectItem key={e} value={e} className="text-[10px] tabular">{fmtExp(e)}</SelectItem>
            ))}
          </SelectContent>
        </Select>
      </div>
      ...
      {loading && !chain ? (
        ...
        <EmptyState icon={Sigma} title="No OI data" description="Chain returned no open-interest values." />
      ) : (
        <div className="flex-1 overflow-auto">
          ...
          {rows.slice().reverse().map((r) => {
            ...
            const isNearSpot = spot != null && Math.abs(r.strike - spot) < (r.strike * 0.005);
            ...
            return (
              ...
              >
                ...
                <span className={cn(
                  ...
                  isMaxPain ? "text-warning" : isNearSpot ? "text-accent" : "text-text-secondary"
                )}>
                  {r.strike}
                </span>
      ...
      {rows.length > 0 && (
        <div className="grid grid-cols-3 px-2 h-6 items-center border-t border-border/60 text-[9px] tabular text-text-muted">
          <span>P/C OI <span className={cn(
            "font-medium",
            pcr != null && pcr > 1.2 ? "text-down" : pcr != null && pcr < 0.7 ? "text-up" : "text-text-secondary"
          )}>{pcr != null ? pcr.toFixed(2) : "—"}</span></span>
          <span className="text-center">Max Pain <span className="text-warning font-medium">{maxPain ?? "—"}</span></span>
          <span className="text-right">spot <span className="text-text-primary font-medium">{spot != null ? spot.toFixed(2) : "—"}</span></span>
        </div>
      )}
    </div>
  );
