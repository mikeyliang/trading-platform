...
export function BacktestChart({ result }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const barsResp = await api
        .bars(result.symbol, result.timeframe, daysBetween)
        .catch(() => null);
      if (cancelled) return;
      ...
      if (showSmi && smiEl && result.smi_data && result.smi_data.length) {
        ...
        const onPrice = (range: any) =>
          range && smiChart!.timeScale().setVisibleLogicalRange(range);
        const onSmi = (range: any) =>
          range && priceChart!.timeScale().setVisibleLogicalRange(range);
    ...
    const ro = new ResizeObserver(() => {
      ...
      if (smiChart && smiEl)
        smiChart.applyOptions({
          width: smiEl.clientWidth,
          height: smiEl.clientHeight,
        });
      if (eqChart && eqEl)
        eqChart.applyOptions({
          width: eqEl.clientWidth,
          height: eqEl.clientHeight,
        });
    });
    ...
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-3 px-1">
        <div className="text-[10px] uppercase tracking-wider text-text-muted">
          {result.symbol} · {result.timeframe} · {result.start_date} →{" "}
          {result.end_date}
        </div>
      ...
      <div
        ref={priceRef}
        className="w-full h-72 bg-bg border border-border rounded-md overflow-hidden"
      />
